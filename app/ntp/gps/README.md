...
GPS卫星有精密时钟源，GPS模块依靠接收到信号时间差来计算位置，同时GPS模块可以通过以下两种方式对外输出时间信息：
...
我是从taobao 深圳北天通讯 购买的GPS/北斗双模BS-70DU接收器。其中USB口用来给模块供电，RS232口用来通信。
...
我本来计划使用atom处理的小盒子做ntp服务器，测试时发现无法成功，可能与atom处理的CPU自动降频有关。

后来找了一台比较老的服务器，顺利完成。

## 3.1 安装OS和软件

我使用的是CentOS 7，安装系统后，运行以下命令安装需要的软件：
...
此时，从其他计算机，执行 ntpdate ntp服务器IP地址，可以完成对操作。
